...
public class AntiShellSystemAI implements AutofireAIPlugin {
	...
	public AntiShellSystemAI(final WeaponAPI weapon, final ShipAPI ship) {
		this.weapon = weapon;
		this.ship = ship;
	...
	public void advance(float amount) {
		...
		if (!this.engine.getCustomData().containsKey(prefix + this.weapon.toString())) {
			this.engine.getCustomData().put(prefix + this.weapon.toString(), this.tracks);
		}
		// initalize data storing the current active weapon among the same weapons of
		// the ship
		if (!this.engine.getCustomData().containsKey(prefix + this.ship.toString() + this.weapon.getId())) {
			this.engine.getCustomData().put(prefix + this.ship.toString() + this.weapon.getId(), this.weapon);
		}
		// initalize data storing the all weapons of the same kind on the ship
		if (!this.engine.getCustomData()
				.containsKey(prefix + this.ship.toString() + this.weapon.getId() + "list")) {
			...
			this.engine.getCustomData().put(prefix + this.ship.toString() + this.weapon.getId() + "list", weapons);
		}

		this.checkIsAutoFire();
		this.updateTracks();

		// if (this.targetShip != null && this.weapon == (WeaponAPI)
		// this.engine.getCustomData()
		// .get(prefix + this.ship.toString() + this.weapon.getId())) {
		// this.fireTimer.advance(amount);
		// }

		if (this.targetProjectile == null) {
			this.targetProjectile = this.selectTargetProjectile();
			// this.tempTargetProjectile =
			// MagicTargeting.randomMissile((CombatEntityAPI)this.ship,
			// MagicTargeting.missilePriority.DAMAGE_PRIORITY, this.weapon.getLocation(),
			// this.weapon.getArcFacing(), Integer.valueOf(weaponArc), Integer.valueOf((int)
			// this.weapon.getRange() + 200));
			// if (this.isTargetMissileValid(this.tempTargetMissile)) {
			// this.targetMissile = this.tempTargetMissile;
			// return;
			// }
			// if (!this.isTargetMissileValid(this.tempTargetMissile)) {
			// this.tempTargetMissile =
			// MagicTargeting.randomMissile((CombatEntityAPI)this.ship,
			// MagicTargeting.missilePriority.RANDOM, this.weapon.getLocation(),
			// this.weapon.getArcFacing(), Integer.valueOf(weaponArc), Integer.valueOf((int)
		...
		// this.targetShip = MagicTargeting.pickShipTarget(this.ship,
		// MagicTargeting.targetSeeking.FULL_RANDOM , Integer.valueOf((int)
		// this.weapon.getRange()), Integer.valueOf(weaponArc), Integer.valueOf(100),
		...
		// }
		// this.targetShip = MagicTargeting.pickShipTarget(this.ship,
		// MagicTargeting.targetSeeking.LOCAL_RANDOM , Integer.valueOf((int)
		// this.weapon.getRange()), Integer.valueOf(weaponArc), Integer.valueOf(25),
		// Integer.valueOf(25), Integer.valueOf(25), Integer.valueOf(25),
		// Integer.valueOf(25));
		// }
		// if (this.targetShip != null) {
		// if (!this.targetShip.isAlive() || Misc.getDistance(this.weapon.getLocation(),
		// this.targetShip.getLocation()) > this.weapon.getRange()) {
		// this.targetShip = null;
		// return;
		// }
		// }
	...
	public Vector2f getTarget() {
		if (this.targetProjectile != null) {
			return this.engine.getAimPointWithLeadForAutofire((CombatEntityAPI) this.ship, 1f,
					(CombatEntityAPI) this.targetProjectile, this.weapon.getProjectileSpeed());
	...
	public DamagingProjectileAPI selectTargetProjectile() {
		...
		List<DamagingProjectileAPI> projectiles = CombatUtils.getProjectilesWithinRange(this.weapon.getLocation(),
				this.weapon.getRange() * 1.75f);
		...
		for (DamagingProjectileAPI p : projectiles) {
			...
			// if (Misc.getAngleDiff(p.getFacing(),
			// Misc.getAngleInDegrees(p.getLocation(), this.ship.getLocation())) > 90f) {
			// continue;
			// }
			...
			if ((Math.pow((2 * m * n - 2 * k - 2 * n), 2)
					- 4 * (m * m + 1) * (n * n - 2 * n * k - r * r + h * h + k * k)) < 0) {
				continue;
			}
			// if (this.weapon.getSlot().isTurret()) {
			// if (Misc.getAngleDiff(this.weapon.getArcFacing(),
			// Misc.getAngleInDegrees(this.weapon.getLocation(), p.getLocation()) ) > 30f) {
			// continue;
			// }
			// }
			validTargets.add(p);
		...
		for (DamagingProjectileAPI p : validTargets) {
			if (selectedTarget == null) {
				...
				if (Misc.getDistance(selectedTarget.getLocation(), this.weapon.getLocation()) > Misc
						.getDistance(p.getLocation(), this.weapon.getLocation())) {
					selectedTarget = p;
				}
			}
		}
	...
	public boolean shouldFire() {
		...
		if (this.targetProjectile != null && this.weapon == this.engine.getCustomData()
				.get(prefix + this.ship.toString() + this.weapon.getId())) {
			if (isTargetProjectileValid(this.targetProjectile)) {
				if (!tracks.contains(this.targetProjectile)) {
					...
					// log.info("Targetting " + Misc.getDistance(this.weapon.getLocation(),
					// this.targetMissile.getLocation()) + " by " + this.weapon.getId() + " " +
					// this.getMinLaunchDist());
				}
				return true;
			}
		}
	...
	public boolean isTargetProjectileValid(DamagingProjectileAPI projectile) {
		...
		if (projectile.getSpawnType() == ProjectileSpawnType.BEAM) {
			return false;
		}
		if (projectile.getProjectileSpec() != null) {
			if (projectile.getProjectileSpec().isPassThroughMissiles()
					|| projectile.getCollisionClass() == CollisionClass.NONE) {
				return false;
			}
		}
		...
		if (this.engine.getCustomData().containsKey(prefix + projectile.toString())) {
			for (MissileAPI m : (List<MissileAPI>) this.engine.getCustomData()
					.get(prefix + projectile.toString())) {
	...
	public void checkIsAutoFire() {
		...
		WeaponAPI currentWeapon = (WeaponAPI) this.engine.getCustomData()
				.get(prefix + this.ship.toString() + this.weapon.getId());
		if (this.ship != this.engine.getPlayerShip()
				|| this.ship.getWeaponGroupFor(currentWeapon) != this.ship.getSelectedGroupAPI()) {
		...
		if (!this.tracks.isEmpty()) {
			...
			for (DamagingProjectileAPI d : this.tracks) {
				if (this.isProjectileAlive(d)) {
					...
					if (this.engine.getCustomData().containsKey(prefix + d.toString())) {
						this.engine.getCustomData().remove(prefix + d.toString());
					}
			...
			for (DamagingProjectileAPI d : this.tracks) {
				if (this.engine.getCustomData().containsKey(prefix + d.toString())) {
					...
					this.engine.getCustomData().put(prefix + d.toString(), birds);
				}
			}
		}
		return;
